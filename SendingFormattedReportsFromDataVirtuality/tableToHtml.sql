alter VIRTUAL PROCEDURE x_utils.tableToHtml( 
	IN tableName string NOT NULL Options (Annotation 'Table or view which has to be converted') 
)
RETURNS( 
	html string options (Annotation 'Html converted from a table or view')
)  Options (Annotation 'Converts a given table or view to html')
AS
BEGIN
	...
	declare clob qry = 
	 'select 
	 		xmlelement(NAME "table",
	 			XMLATTRIBUTES(''1'' AS "border", ''Martini'' AS "drink"),
